...
import * as GetPreviewDom from '../src/parts/GetPreviewDom/GetPreviewDom.ts'

test('getPreviewDom should return an array', () => {
  const state = createDefaultState()
  const result = GetPreviewDom.getPreviewDom(state)
...
test('getPreviewDom should return consistent results', () => {
  const state = createDefaultState()
  const result1 = GetPreviewDom.getPreviewDom(state)
  const result2 = GetPreviewDom.getPreviewDom(state)
