...
const getTokens = (html: string): Array<{ text: string; type: number }> => {
  return TokenizeHtml.tokenizeHtml(html).map((t: Readonly<{ text: string; type: number }>) => ({ text: t.text, type: t.type }))
}
...
test('tokenizeHtml should handle script content with multiple < operators', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('for'))
...
test('tokenizeHtml should handle script content with > operator', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('if'))
  expect(contentToken).toBeDefined()
...
test('tokenizeHtml should handle script content with template literals', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('const'))
  expect(contentToken).toBeDefined()
...
test('tokenizeHtml should handle script with attributes', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('console'))
  expect(contentToken).toBeDefined()
...
test('tokenizeHtml should handle script tag followed by other content', () => {
  ...
  const scriptContent = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('var'))
  expect(scriptContent).toBeDefined()
  expect(scriptContent!.text).toBe('var x = 1;')
  // Should also have div tokens after
  const divStart = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.TagNameStart && t.text === 'div')
...
test('tokenizeHtml should handle script with multiline content', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('function'))
  expect(contentToken).toBeDefined()
...
test('tokenizeHtml should handle script with HTML-like strings', () => {
  ...
  const tokens = getTokens(`<script>${js}</script>`)
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('document'))
  expect(contentToken).toBeDefined()
  expect(contentToken!.text).toBe(js)
})
...
test('tokenizeHtml should handle style content with angle brackets', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('div'))
  expect(contentToken).toBeDefined()
...
test('tokenizeHtml should handle style tag with normal CSS', () => {
  ...
  const tokens = getTokens(`<style>${css}</style>`)
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text === css)
  expect(contentToken).toBeDefined()
})
...
test('tokenizeHtml should handle script with no closing tag', () => {
  ...
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('var'))
  expect(contentToken).toBeDefined()
...
test('tokenizeHtml should handle script between other elements', () => {
  ...
  const contentTokens = tokens.filter((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content)
  expect(contentTokens.map((t: Readonly<{ text: string; type: number }>) => t.text)).toEqual(['before', 'if (a < b) {}', 'after'])
})

test('tokenizeHtml should handle script with arrow functions', () => {
  const js = 'const fn = (a) => a < 10 ? a : 10'
  const tokens = getTokens(`<script>${js}</script>`)
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('=>'))
  expect(contentToken).toBeDefined()
  expect(contentToken!.text).toBe(js)
})
...
test('tokenizeHtml should handle script with regex containing angle brackets', () => {
  ...
  const tokens = getTokens(`<script>${js}</script>`)
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('const re'))
  expect(contentToken).toBeDefined()
  expect(contentToken!.text).toBe(js)
})
...
test('tokenizeHtml should handle script with JSX-like content', () => {
  ...
  const tokens = getTokens(`<script>${js}</script>`)
  const contentToken = tokens.find(
    (t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('createElement'),
  )
  expect(contentToken).toBeDefined()
  expect(contentToken!.text).toBe(js)
})
...
test('tokenizeHtml should handle script with generics-like syntax', () => {
  ...
  const tokens = getTokens(`<script>${js}</script>`)
  const contentToken = tokens.find((t: Readonly<{ text: string; type: number }>) => t.type === HtmlTokenType.Content && t.text.includes('Array'))
  expect(contentToken).toBeDefined()
  expect(contentToken!.text).toBe(js)
})
