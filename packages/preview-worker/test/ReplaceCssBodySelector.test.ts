...
test('should replace simple body selector', () => {
  ...
  expect(result).toBe('.Preview .Html .Body {  color: red;  }')
})

test('should replace body in combined selectors', () => {
  const css = 'body, div { margin: 0; }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body, .Preview div {  margin: 0;  }')
})

test('should replace multiple body selectors', () => {
  const css = 'body { color: red; } body p { margin: 10px; }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body {  color: red;  } .Preview .Html .Body p {  margin: 10px;  }')
})
...
test('should handle body with pseudo-classes', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body:hover {  background: yellow;  }')
})

test('should handle body with pseudo-elements', () => {
  const css = 'body::before { content: ""; }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body::before {  content: "";  }')
})

test('should handle body with descendant selectors', () => {
  const css = 'body > * { box-sizing: border-box; }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body > * {  box-sizing: border-box;  }')
})
...
test('should handle body with newlines', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe(`.Preview .Html .Body {
  color: red;
  background: white;
 }`)
})
...
test('should handle multiple body selectors in different contexts', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body {  font-size: 16px;  } .container .Preview .Html .Body {  display: flex;  }')
})
...
test('should handle both body and universal selectors', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(result).toBe('.Preview .Html .Body {  color: black;  } .Preview * {  box-sizing: border-box;  }')
})
...
test('should replace html and body in combined selectors', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('.Preview .Html, .Preview .Html .Body, .Preview main { margin: 0; }')
})
...
test('should handle selector list with is pseudo class commas', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('.Preview :is(.Preview .Html .Body, .Preview .Html, main) { display: block; }')
})

test('should handle attribute selector containing comma', () => {
  const css = '[data-value="a,b"], body { color: red; }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('.Preview [data-value="a,b"], .Preview .Html .Body { color: red; }')
})
...
test('should keep comments inside declarations', () => {
  ...
  expect(normalizeWhitespace(result)).toContain('.Preview .Html .Body { color: red; /* keep-me */ background: white; }')
})

test('should process rules separated by comments', () => {
  const css = 'body { color: red; } /* between */ div { color: blue; }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toContain('.Preview .Html .Body { color: red; }')
...
test('should scope selectors in media query', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('@media (max-width: 500px) { .Preview .Html .Body { color: red; } .Preview div { margin: 0; } }')
})

test('should scope selectors in supports query', () => {
  const css = '@supports (display: grid) { body { display: grid; } }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('@supports (display: grid) { .Preview .Html .Body { display: grid; } }')
})

test('should scope selectors in container query', () => {
  const css = '@container sidebar (min-width: 300px) { body { font-size: 12px; } }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('@container sidebar (min-width: 300px) { .Preview .Html .Body { font-size: 12px; } }')
})
...
test('should scope selectors in scope blocks', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('@scope (.card) { .Preview .Html .Body { padding: 8px; } }')
})

test('should scope selectors in document blocks', () => {
  const css = '@document url-prefix("https://example.com") { body { color: green; } }'
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('@document url-prefix("https://example.com") { .Preview .Html .Body { color: green; } }')
})
...
test('should recurse through nested media and supports blocks', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('@media (min-width: 500px) { @supports (display: flex) { .Preview .Html .Body { display: flex; } } }')
})
...
test('should preserve keyframes without scoping inside keyframes', () => {
  ...
  expect(normalizeWhitespace(result)).toContain('.Preview .Html .Body { animation: fade 1s; }')
})
...
test('should preserve font-face blocks', () => {
  ...
  expect(normalizeWhitespace(result)).toContain('.Preview .Html .Body { font-family: test; }')
})
...
test('should preserve page blocks', () => {
  ...
  expect(normalizeWhitespace(result)).toContain('.Preview .Html .Body { color: black; }')
})
...
test('should support css nesting syntax with body as parent selector', () => {
  ...
  const result = ReplaceCssBodySelector.replaceCssBodySelector(css)
  expect(normalizeWhitespace(result)).toBe('.Preview .Html .Body { & .title { color: red; } }')
})
...
test('should scope multiple top-level at-rule and normal rule combinations', () => {
  ...
  expect(normalizeWhitespace(result)).toContain('@media (min-width: 1px) { .Preview .Html .Body { margin: 0; } }')
