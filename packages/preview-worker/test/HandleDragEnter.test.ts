...
test('handleDragEnter should return state unchanged when hdId is empty', () => {
  const state = createState(1)
  const result = HandleDragEnter.handleDragEnter(state, '')
  expect(result).toBe(state)
})

test('handleDragEnter should return state unchanged when no happy-dom state exists', () => {
  const state = createState(1)
  const result = HandleDragEnter.handleDragEnter(state, '0')
  expect(result).toBe(state)
})

test('handleDragEnter should return state unchanged when element not found in map', () => {
  setupHappyDom(1, '<body><div>hello</div></body>', [])
  const state = createState(1)
  const result = HandleDragEnter.handleDragEnter(state, '999')
  expect(result).toBe(state)
})

test('handleDragEnter should dispatch dragenter and update state', () => {
  ...
  const state = createState(1)
  const result = HandleDragEnter.handleDragEnter(state, dropzoneHdId)

  expect(result).not.toBe(state)
  const textNode = result.parsedDom.find((n: any) => n.text === 'entered')
